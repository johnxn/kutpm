...
#if 0
int stuff_inoutbuf_firsttime(void) {
    memset(inout_buff, 0, sizeof(inout_buff));
    UINT32 in_size = 0xffffffff;
    memcpy(inout_buff, &in_size, sizeof(UINT32));
    return 0;
}

int stuff_inoutbuf_startup(void) {
    memset(inout_buff, 0, sizeof(inout_buff));
    UINT32 in_size = 2 + 4 + 4 + 2;
    if (in_size + 4 > sizeof(inout_buff)) return -1;
    memcpy(inout_buff, &in_size, 4);

    BYTE *ptr = inout_buff + 4;
...
int stuff_inoutbuf_get_random(UINT32 size) {
    int command;
    memset(inout_buff, 0, INOUTBUF_LEN);
    command = COMMAND_GET_RANDOM;
    memcpy(inout_buff, &command, sizeof(int));
    memcpy(inout_buff+sizeof(int), &size, sizeof(int));
    return 0;
}
...
int get_random_info(
    ...
    int size;
    memcpy(&size, inout_buff, sizeof(int));
    if (size == 0xffffffff) {
        //printf("0xffffffff got.\n");
        return -1;
    }
    memcpy(out, inout_buff+sizeof(int), size);
    *res = TPM_SUCCESS;
    return 0;
}
...
int get_oiap_info(
    ...
    UINT32 length;
    BYTE *ptr;
    memcpy(&length, inout_buff, sizeof(UINT32));
    if (length == 0xffffffff) {
        return -1;
    }
    ptr = inout_buff + sizeof(UINT32);
    TPM_TAG tag;
    UINT32 size;
...
int stuff_inoutbuf_createcrapkey(
    ...
    BYTE *ptr2 =  inout_buff + 4 + 2 + 4 + 4; //pass in_size, tag, parmSize, ordinal;
...
int stuff_inoutbuf_bind(
    ...
    memcpy(inout_buff, &command, sizeof(int));
    memcpy(inout_buff+sizeof(int), pubkey, sizeof(RSA_PUBLIC_KEY));
    memcpy(inout_buff+sizeof(int)+sizeof(RSA_PUBLIC_KEY), &inputSize, sizeof(UINT32));
    memcpy(inout_buff+sizeof(int)+sizeof(RSA_PUBLIC_KEY)+sizeof(UINT32), input, inputSize);
    return 0;
}
int get_bind_info(
    TPM_RESULT *res,
    ...
    //printf_buf("inout_buff", inout_buff, sizeof(inout_buff));
    memcpy(outputSize, inout_buff, sizeof(UINT32));
    if (*outputSize == 0xffffffff) {
        //printf("0xffffffff got.\n");
        //*res = TPM_FAIL;
        return -1;
    }
    memcpy(output, inout_buff+sizeof(UINT32), *outputSize);
    *res = TPM_SUCCESS;
    return 0;
}
